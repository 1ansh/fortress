function [final_path] = call_rrt(map,source,goal)
% map = 1 - blue1;
% source = [round(brcent(2)) round(brcent(1))];
% goal = [round(targetcent(2)) round(targetcent(1))];

stepsize=50; % size of each step of the RRT
disTh=30; % nodes closer than this threshold are taken as almost the same
...
if display, imshow(map);rectangle('position',[1 1 size(map,2)-1 size(map,1)-1],'edgecolor','k'); end
...
[ final_path ] = smooth_path( path ,map );
% waitforbuttonpress;
figure;
imshow(map);
rectangle('position',[1 1 size(map)-1],'edgecolor','k');
